...
export const meta: MetaFunction = () => [
	{
		title: 'Nueva nota crédito de venta | Villing',
		description: 'Nueva nota crédito de venta',
	},
];
...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	return json({
		invoices: invoices.map(invoice => ({
			...invoice,
			url: `/invoices/duplicate?origin=legalInvoice&destination=creditNote&id=${invoice.id}`,
		...
		<PageWrapper>
			...
			<div className="flex gap-4 justify-between items-end mb-4">
				<div className="flex-1">
					<h2 className="mb-1">Nueva nota crédito</h2>
					<p className="text-gray-500 text-sm leading-none">
						Selecciona una factura para relacionarla a la nueva nota crédito.
					</p>
				</div>
			</div>
...
export function ErrorBoundary() {
	return (
		<RouteErrorBoundary message="Lo sentimos, hubo un error inesperado con la nota crédito. Por favor, vuelve a intentarlo o contacta con nosotros." />
	);
}
