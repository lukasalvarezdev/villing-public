...
import { catchCognitoError, getUser } from '~/utils/auth.server';
...
export async function loader({ request }: ActionFunctionArgs) {
	...
	invariant(token && email, 'Token and email are required');

	const currentUser = await getUser(request);

	if (currentUser && currentUser?.email !== email) {
		return redirect('/home?msg=unauthorized');
	}
	...
	try {
		const result = await __prisma.$transaction(async tx => {
			const user = await tx.user.findUnique({
				...
				select: { id: true, tokenExpiresAt: true, confirmedAt: true },
			});

			if (!user) return { layout: 'error', message: 'user_not_found' } as const;
			if (user.tokenExpiresAt && new Date() > user.tokenExpiresAt) {
				return { layout: 'error', message: 'token_expired' } as const;
			}
			if (user.confirmedAt) {
