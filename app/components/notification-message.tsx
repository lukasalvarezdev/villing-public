		...
		<AnimatePresence>
			{show ? (
				<motion.div
					...
					className={cn(
						'print:hidden',
						'text-inverse pointer-events-none fixed left-0 right-8 z-50 px-5vw',
						{
				...
				>
					<div
						className={cn('mx-auto flex w-full max-w-8xl', {
					...
					>
						<div
							className={cn(
								'pointer-events-auto relative max-w-xl p-6 pr-14',
								'bg-white shadow-md rounded-lg border border-gray-100',
								'border-l-8 border-l-primary-600',
							)}
						>
							{typeof controlledVisible === 'undefined' || onDismiss ? (
								<button
									...
									className="text-gray-900 hover:text-inverse focus:text-inverse absolute right-4 top-8 rotate-45 transform"
								>
